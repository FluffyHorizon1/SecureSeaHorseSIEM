...
struct AppConfig {
    ...
    size_t get_size(const std::string& key, size_t def) const {
        try {
            return data.count(key) ? std::stoull(data.at(key)) : def;
        }
        catch (...) {
            return def;
        }
    ...
    // Allow CLI overrides to merge into config
...
// =============================================================================
// PHASE 1: CLI ARGUMENT PARSING (cross-platform, no external deps)
// =============================================================================
struct CliArgs {
    std::string config_path;            // --config / -c
    std::map<std::string, std::string> overrides;  // --set key=value
    bool show_help = false;             // --help / -h
    bool show_version = false;          // --version / -v

    // Merge CLI overrides into AppConfig (CLI wins over file)
...
inline void print_client_usage(const char* prog) {
    std::cout << "Usage: " << prog << " [OPTIONS]\n"
        ...
        << "\nExamples:\n"
        << "  " << prog << " --config /etc/ssh/client.conf\n"
        << "  " << prog << " -c custom.conf --set server_ip=10.0.0.5 --set port=9999\n";
}
    ...
    for (int i = 1; i < argc; i++) {
        ...
        if (arg == "-h" || arg == "--help") {
            ...
            if (eq != std::string::npos) {
                args.overrides[kv.substr(0, eq)] = kv.substr(eq + 1);
            }
        }
        else {
...
public:
    /// @param base_ms  Starting delay (default 1000ms = 1s)
    /// @param max_ms   Maximum delay cap (default 60000ms = 60s)
    ...
    /// Returns the next delay in milliseconds, applying exponential growth + jitter
    int next_delay_ms() {
        // 2^attempt * base, capped at max
        int exp_delay = base_ms_ * (1 << std::min(attempt_, 20)); // cap shift to avoid overflow
    ...
    /// Get current attempt count (useful for logging)
...
private:
    ...
    std::string format_entry(const LogEntry& entry) {
        ...
        ss << "[" << std::put_time(std::localtime(&t), "%Y-%m-%d %H:%M:%S") << "] "
            << level_str(entry.level) << entry.message;
        return ss.str();
    }
    ...
    void rotate_logs() {
        ...
        std::string oldest = base_filename_ + "." + std::to_string(max_rotated_files_);
    ...
    void worker_loop() {
        while (true) {
            std::queue<LogEntry> batch;
            {
                ...
                // Drain the entire queue into a local batch (non-blocking for producers)
                std::swap(batch, queue_);
            }

            // Write batch outside the lock
            while (!batch.empty()) {
                std::string formatted = format_entry(batch.front());
                batch.pop();
...
public:
    ...
    /// @param max_file_size    Max bytes before rotation (default 10MB)
    /// @param max_files        Number of rotated files to keep (default 5)
    /// @param echo_console     Also write to stdout (default true)
    AsyncLogger(const std::string& filename = "",
        ...
        max_rotated_files_(max_files),
        current_file_size_(0),
        echo_to_console_(echo_console)
    {
        if (!filename.empty()) {
    ...
    ~AsyncLogger() {
        // Signal shutdown and flush remaining messages
        {
    ...
    void log(Level level, const std::string& msg) {
        {
            std::lock_guard<std::mutex> lock(queue_mutex_);
            if (!running_) return;
            queue_.push({ level, msg, std::chrono::system_clock::now() });
        }
        cv_.notify_one();
    }

    /// Convenience: flush pending messages synchronously (blocks until queue is drained)
    void flush() {
        // Enqueue a sentinel and wait
        std::mutex m;
        ...
        {
            std::lock_guard<std::mutex> lock(queue_mutex_);
            // We push a special log entry and then wait for it to be processed
            // This is a simple approach — the worker will process everything before it
            queue_.push({ INFO, "__FLUSH_SENTINEL__", std::chrono::system_clock::now() });
        }
        cv_.notify_one();

        // Spin-wait for queue to empty (simple but effective for infrequent flush calls)
        while (true) {
