...
struct FimEntry {
    ...
    int64_t     mtime_epoch = 0;  // Last modification time (Unix epoch seconds)
...
struct FimChange {
    FimChangeType type      = FimChangeType::FIM_ADDED;
    std::string   path;
...
// Single-open design: caller passes in an upper size bound. We open the file
// once, verify the size as we stream, and abort the hash (returning "") if
// the file grew beyond the bound. This eliminates the TOCTOU race where a
// fs::file_size() check and a subsequent open() could observe different files.
// On success, actual_size_out receives the exact streamed byte count.
// =============================================================================
inline std::string sha256_file(const std::string& filepath,
                               uint64_t max_bytes = 0,
                               uint64_t* actual_size_out = nullptr) {
    ...
    while (file.read(buf, sizeof(buf)) || file.gcount() > 0) {
        ...
        total += chunk;
        // Enforce size bound on the fly so we never hash a file that was
        // enlarged between probe and hash.
        if (max_bytes > 0 && total > max_bytes) {
            EVP_MD_CTX_free(ctx);
            return "";
        }
        if (EVP_DigestUpdate(ctx, buf, chunk) != 1) {
            EVP_MD_CTX_free(ctx);
            return "";
        }
    }
