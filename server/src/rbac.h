...
public:
                      ...
                      const std::string& source_ip)
    {
        ...
        // Phase 31 (review 4.6): constant-time comparison. std::string::operator!=
        // short-circuits on the first differing byte and is timing-observable.
        const std::string& stored = it->second.pw_hash;
        bool pw_ok = !attempt.empty()
            && attempt.size() == stored.size()
            && CRYPTO_memcmp(attempt.data(), stored.data(), attempt.size()) == 0;
        if (!pw_ok) {
            lock.unlock();
    ...
    Claims verify_jwt(const std::string& jwt) const {
        ...
        // Phase 31 (review 4.6): constant-time signature comparison.
...
private:
    ...
    // Phase 31 (review 4.4): key-stretched password hashing. The prior scheme
    // was a single-pass SHA-256(salt||pw) -- salted but not stretched, so a
    // leaked users.db was brute-forceable at GPU speed. Use PBKDF2-HMAC-SHA256.
    // Changing kPbkdf2Iters invalidates existing hashes; this is greenfield
    // (no users exist until --create-admin), so that is acceptable.
    static constexpr int kPbkdf2Iters = 600000;
    static std::string hash_password(const std::string& salt, const std::string& pw) {
        unsigned char out[32];
        // PBKDF2 over the password with the per-user salt (the salt is the
        // random hex string stored alongside; its bytes are the PBKDF2 salt).
        if (PKCS5_PBKDF2_HMAC(pw.data(), static_cast<int>(pw.size()),
                reinterpret_cast<const unsigned char*>(salt.data()),
                static_cast<int>(salt.size()),
                kPbkdf2Iters, EVP_sha256(),
                static_cast<int>(sizeof(out)), out) != 1) {
            return "";  // hashing failure -> empty, never matches a stored hash
        }
