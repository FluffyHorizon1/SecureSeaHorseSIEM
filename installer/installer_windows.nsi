; =============================================================================
; SecureSeaHorse SIEM v5.0.0 -- Windows Installer (NSIS)
; =============================================================================
; Build: makensis installer\installer_windows.nsi   (run from repo root)
...
OutFile "SecureSeaHorse-v5.0.0-Setup.exe"
...
Section "SIEM Server" SEC_SERVER
    ...
    File /oname=seahorse-server.exe "server\build\Release\SeaHorseServer.exe"

    SetOutPath "$INSTDIR\server\config"
    File "server\config\server.conf"
    File "server\config\rules.conf"
...
Section "SIEM Client Agent" SEC_CLIENT
    ...
    File /oname=seahorse-client.exe "client\build\Release\SeaHorseClient.exe"

    SetOutPath "$INSTDIR\client\config"
    File "client\config\client.conf"
