#!/bin/bash
# =============================================================================
# SecureSeaHorse SIEM v5.0.0 -- Linux Installer
# =============================================================================
# Usage:
...
build_from_source() {
    ...
    SERVER_BUILD="$SCRIPT_DIR/server/build"
    CLIENT_BUILD="$SCRIPT_DIR/client/build"
    mkdir -p "$SERVER_BUILD" "$CLIENT_BUILD"
    cd "$SERVER_BUILD"
    log "Running CMake..."
    cmake "$SCRIPT_DIR/server" -DCMAKE_BUILD_TYPE=Release && make -j"$(nproc)"
    log "Server built. Building client..."
    cd "$CLIENT_BUILD"
    cmake "$SCRIPT_DIR/client" -DCMAKE_BUILD_TYPE=Release
    log "Compiling..."
    make -j"$(nproc)"
    log "Build complete."
    cd "$SCRIPT_DIR"
...
install_server() {
    ...
    if [ -f "$SCRIPT_DIR/server/build/SeaHorseServer" ]; then
        cp "$SCRIPT_DIR/server/build/SeaHorseServer" "$PREFIX/server/bin/seahorse-server"
    else
        warn "Server binary not found in build/. Run with 'build' option first."
    fi

    if [ ! -f "$PREFIX/server/config/server.conf" ]; then
        cp "$SCRIPT_DIR/server/config/server.conf" "$PREFIX/server/config/"
        log "Installed default server.conf"
    else
        cp "$SCRIPT_DIR/server/config/server.conf" "$PREFIX/server/config/server.conf.new"
        warn "Existing server.conf preserved. New saved as server.conf.new"
    fi
    [ ! -f "$PREFIX/server/config/rules.conf" ] && cp "$SCRIPT_DIR/server/config/rules.conf" "$PREFIX/server/config/"

    # v5.0.0 runtime data: sigma rules, feed samples, USB whitelist, update manifest
    mkdir -p "$PREFIX/server/config/sigma" "$PREFIX/server/updates"
    cp -n "$SCRIPT_DIR/server/config/sigma/"*.yml "$PREFIX/server/config/sigma/" 2>/dev/null || true
    cp -n "$SCRIPT_DIR/server/feeds/"*.csv "$PREFIX/server/config/feeds/" 2>/dev/null || true
    [ ! -f "$PREFIX/server/config/usb_whitelist.csv" ] && cp "$SCRIPT_DIR/server/config/usb_whitelist.csv" "$PREFIX/server/config/" || true
    cp -n "$SCRIPT_DIR/server/updates/manifest.json" "$PREFIX/server/updates/" 2>/dev/null || true
...
install_client() {
    ...
    if [ -f "$SCRIPT_DIR/client/build/SeaHorseClient" ]; then
        cp "$SCRIPT_DIR/client/build/SeaHorseClient" "$PREFIX/client/bin/seahorse-client"
    else
        warn "Client binary not found in build/. Run with 'build' option first."
    fi

    if [ ! -f "$PREFIX/client/config/client.conf" ]; then
        cp "$SCRIPT_DIR/client/config/client.conf" "$PREFIX/client/config/"
    else
        cp "$SCRIPT_DIR/client/config/client.conf" "$PREFIX/client/config/client.conf.new"
    fi
